...
def test_test_database_schema_matches_requirements(temp_db):
    with sqlite3.connect(temp_db) as con:
        ...
        assert {"issues", "comments", "attachments", "issue_contributing_users", "issue_history"}.issubset(tables)
        ...
        contributing_cols = [row[1] for row in con.execute("PRAGMA table_info(issue_contributing_users)")]
        assert contributing_cols == [
            "id", "issue_id", "contributing_username", "contributed_by_username", "created_at",
        ]
